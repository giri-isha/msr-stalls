...
> **DELIVERED — 14 Sep 2026.** Every task below is done and every box ticked.
> Kept as the record of what the merge left unfinished and how it was closed.
> Verified at the end: typecheck clean across all three workspaces, 136 + 244 +
> 115 tests green, `lint:boundaries` clean.
...
- [x] **Step 1: Run the failing typecheck and capture the list**

Run: `npm run typecheck --workspace=apps/api 2>&1 | grep 'error TS'`
Expected: 9 errors across the five files above.

- [x] **Step 2: `helpers/plan.ts` — categories come from the edition**
...
- [x] **Step 2: Electrical — the bay filter is loaded, not constant**
...
- [x] **Step 3: Charges panel loses the deposits, gains what the schema has**
...
- [x] **Step 5: The advance is shown per bay, not once in the header**
...
- [x] **Step 2: Run them, watch them fail with 404**
...
- [x] **Step 4: Delete `quoteInclude`**

Nothing references it, in its own file or anywhere else.

- [x] **Step 5: Client functions for each**
...
- [x] **Step 1: Reset and migrate both databases**
...
- [x] **Step 2: Seed and check the seed still matches the schema**
...
- [x] **Step 1: Traceability doc covers the newly routed writes**

Each of the eight routes from Task 6 against the requirement it serves.

- [x] **Step 2: README's module map drops the superseded files**
...
- [x] **Step 3: Migration-to-host doc reflects the self-contained web module**
