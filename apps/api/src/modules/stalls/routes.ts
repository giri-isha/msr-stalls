...
// ⚠️ `activeEdition` itself is deliberately NOT imported here: every staff route
// resolves the season through `activeEditionFor`, which refuses a caller whose
...
import { requirePrivilege, requireAnyPrivilege, requireStaff } from './roles';
...
export function registerStallsStaffRoutes(app: FastifyInstance, deps: StallsDeps): void {
  ...
  // ⚠️ The role LIST above is open to any staff member — the Users screen has to
